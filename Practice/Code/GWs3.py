                ...
                for d in Task:
                    if d['Deadline']<=timetick:
                        if d['GW'] == GW:
                            ...
                            if d['Remain']>0:
                                Task.remove(d)
                                print "Timetick=",timetick,"\tTASK",d['id'],"(GW",d['GW'],",",d['Cnt'],")\t*****MISS*****"
                curU = 0.0
                #===== sched new =====
                for d in Task:
                    if sched_new==None:
...
if __name__=="__main__":
    Task =[{"Exe":0.0,"Period":999,"Deadline":0,'Arrival':-999,"Remain":0,'id':1,"Cnt":0,"OFLD":-999,"OFLDorg":-999,"state":0,"GW":3,"VD":0,"batt":1.0,"uti":0,"time":0}]
    #Task = []
    CC = []
    GW1 = []
    GW2 = []
