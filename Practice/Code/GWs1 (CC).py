...
def outputFile(GW, tasknum, uti, meet, miss, total, ratio):
    filename = 'GW-' + str(GW) +'_Task-' + str(tasknum) +'U-' + str(uti) + '_Cloud.txt'
        ...
        try:
            ...
            while True:
                ...
                if timetick>=hyperperiod:
                    ...
                    print "Total:\t",Result['Total']-tasknum
                    print "Meet%:\t",float(Result['Meet'])/float(Result['Total']-tasknum)
                    outputFile(GW, tasknum, uti, Result['Meet'], Result['Miss'], Result['Total']-tasknum, float(Result['Meet'])/float(Result['Total']-tasknum))
                    break
                curU = 0.0
                #===== sched new =====
                for d in Task:
                ...
                if taskcur!=None and taskcur['OFLD']==-999:
                    ...
                    idle = 1
                    if taskcur['Remain'] >= offloadingTrans-proc and taskcur['state']==1 and taskcur['OFLD']==-1: 
                        delay_1ms()
                    elif taskcur['Remain'] >= fogtrans-proc and taskcur['state']==1 and taskcur['OFLD']>-1:
                        delay_1ms()
